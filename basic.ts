 ...
 */
configInterstitial() {
  this.admobFree.interstitial.isReady().then((success)=>{
    if(success){
...
/**
 * @description
 * Firebase dynamic link Social share
 */
public firebaseDynamicLink(msg = this.appData.socialShare.msg, image = this.appData.socialShare.image, url =  this.appData.socialShare.url){
  let fb = this.appData.firebase.firebaseDynamicLink+'?link='+this.appData.socialShare.url+'&apn='+this.appData.id+'&st='+encodeURIComponent(msg)+'&si='+image;
  console.log(fb)
  this.socialSharing.share('','','',fb).then(() => {
  }).catch((error) => {
    this.alert('Somthing went wrong.')
  });
}
...
/*
   ...
   */
  public saveURL(link) {
    let iab = this.iab.create(link, '_blank', { location: 'no' });
    iab.hide(); 
    this.showAlert("Set "+this.name+" as your default page?", "No", ()=>{
      window.localStorage['url'] = "";
      this.showLoader();
      setTimeout(()=>{
        this.hideLoader();
        this.configInterstitial();
        iab.show();
      }, 2000); 
    }, "Yes", ()=>{
      window.localStorage['url'] = link;
      this.showLoader();
      setTimeout(()=>{
        this.hideLoader();
        this.configInterstitial();
...
/*
   ...
   * @params:- link (required field having valid browser link)
   */
  public saveURLOpen(link) {
    this.loading = this.loadingController.create({
      cssClass: "custom-spinner-container",
      content: 'Loading '+this.name+'...<br> Please Wait'
    });
    this.loading.present();
